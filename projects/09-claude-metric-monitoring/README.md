...
### Standard → Premium (업그레이드 후보)

Premium 활성자 중 해당 채널을 실제로 사용한 사람만 모집단으로 삼고, 한 채널 안에서 사용 강도와 지속성을 동시에 충족한 경우에만 후보로 분류합니다.

여기서 “상위 25% 경계값”은 해당 채널의 Premium 활성 사용자에서 각 지표를 정렬했을 때 상위 25% 구간에 진입하는 기준값입니다. 임계값 자체는 사용 기간과 모집단에 따라 다시 계산합니다.

| 채널 | 강도 | 지속성 |
| --- | --- | --- |
| Chat | 메시지 수 ≥ Premium Chat 활성자의 상위 25% 경계값 | 활성일수 ≥ Premium Chat 활성자의 상위 25% 경계값 |
| Code | 세션 수 ≥ Premium Code 활성자의 상위 25% 경계값 | 활성일수 ≥ Premium Code 활성자의 상위 25% 경계값 |

- 채널 간에는 OR를 적용합니다. Chat 조건 또는 Code 조건 중 하나라도 충족하면 후보입니다.
- 채널 안에서는 AND를 적용합니다. 강도와 지속성을 모두 충족해야 합니다.

```text
Chat 후보 = Chat 강도 ≥ Chat 강도 상위 25% 경계값
         AND Chat 지속성 ≥ Chat 지속성 상위 25% 경계값
Code 후보 = Code 강도 ≥ Code 강도 상위 25% 경계값
         AND Code 지속성 ≥ Code 지속성 상위 25% 경계값
최종 후보 = Chat 후보 OR Code 후보
```

원래는 “Chat 메시지 또는 Code 세션이 Premium 중앙값 이상”인 사용자를 후보로 분류했습니다. Premium 사용자가 코드 중심·채팅 중심의 이중 봉우리 분포를 보여 Chat 중앙값이 낮아졌고 후보가 과다하게 넓어졌습니다. 2026-07-30에 중앙값·단일 지표 기준을 상위 25% 경계값·강도와 지속성의 AND 기준으로 강화해 관리자 검토 가능한 소수 후보로 정제했습니다.

```text
Before: Chat 또는 Code 중앙값 초과 → 후보 과다 추천
After : 채널별 강도·지속성 상위 25% 기준 → 검토 가능한 소수 후보
```

### Premium → Standard (다운그레이드 후보)

다음 세 조건을 모두 충족한 경우 다운그레이드 검토 후보로 분류합니다.

1. Chat 메시지가 0건 또는 Standard 활성자 중앙값 이하
2. Code 세션이 0회 또는 Standard 활성자 중앙값 이하
3. 기간 내 `rate-limit(api_errors)` 도달 0회

이 규칙은 자동 결제나 자동 좌석 변경이 아니라 관리자가 좌석을 검토할 때 사용하는 신호입니다. 업그레이드는 일시적인 spike보다 지속적인 사용 패턴을 우선하고, 다운그레이드는 양쪽 채널의 낮은 사용과 rate-limit 미도달을 함께 확인합니다.
...
- 좌석 상향 후보 기준을 단일 중앙값에서 채널별 강도·지속성 상위 25% 기준으로 바꿔 과다한 후보군을 관리자 검토 가능한 신호로 정리
